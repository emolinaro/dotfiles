...
Use direct clients normally; for isolation use `*-nono` wrappers (`gnhf*`, with `gnhf` as `codex`) that enforce isolated worktree+HOME, `auth.json`-only sync, history-rewrite protection, and hardened env/network.
...
Supported: Apple Silicon macOS (Intel via `nixpkgs.hostPlatform = "x86_64-darwin"`), plus headless Ubuntu 24.04 on x86_64/ARM64 with non-root sudo and auto architecture detection.
...
`./bootstrap.sh` on Ubuntu uses Apt for system packages, Home Manager for user config, auto-selects `x86_64`/`arm64`, enables Docker and `/usr/bin/zsh`, then prompts `codex login` after restart.
...
Only `nixpkgs` and `nixpkgs-linux` are shared; other inputs update independently unless pinned in `packages/treehouse.nix` or `packages/no-mistakes.nix`.
