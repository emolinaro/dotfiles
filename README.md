...
Use normal clients directly, or `*-nono` wrappers (`gnhf*`, `gnhf`→`codex`) for isolated worktree+HOME runs with `auth.json` sync and hardened history/env/network.
...
`./bootstrap.sh` uses Apt (system) and Home Manager (user), auto-detects arch, enables Docker and `/usr/bin/zsh`, then prompts `codex login` after restart.
...
**Homebrew cleanup warning:** `homebrew.onActivation.cleanup = "zap"` removes unlisted Homebrew items on each switch; review `brews`/`casks` before bootstrap/rebuild.
