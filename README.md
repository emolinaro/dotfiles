...
On Ubuntu, `./bootstrap.sh` uses Apt for system packages, Home Manager for user config, auto-detected architecture (`x86_64`/`arm64`), Docker, and `/usr/bin/zsh`, then asks for `codex login` after restart.
