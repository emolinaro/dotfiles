...
`./bootstrap.sh` uses Apt (system) and Home Manager (user), auto-detects architecture, enables Docker and `/usr/bin/zsh`, then prompts for `codex login` after restart.
