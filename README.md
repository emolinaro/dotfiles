...
Use normal clients directly, or `*-nono` wrappers (`gnhf*`, `gnhf`→`codex`) for isolated worktree+HOME runs with `auth.json`-only sync and hardened history/env/network.
